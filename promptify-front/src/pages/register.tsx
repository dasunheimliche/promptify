...
const Register = () => {
  ...
  const { data: { getInvalidUsernames: invalidUsernames } = {} } =
    useQuery<InvaludUsernameList>(INVALID_USERNAMES);
  ...
  const isPasswordOk = useMemo(() => {
    return !(
      inputs.password !== inputs.password2 || inputs.password.length < 1
    );
      ...
      <div className={style["left-side"]}>
        <div className={style.card}>
          ...
          <form className={style.form} onSubmit={signin}>
            ...
            />
            <div
              className={!isUsernameOk ? style.invalidUser : undefined}
            ></div>
