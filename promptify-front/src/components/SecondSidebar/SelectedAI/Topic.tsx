...
export default function TopicTile({
  ...
  const [deleteTopic, { loading: DTloading }] = useMutation(DELETE_TOPIC, {
    update: (cache, response) => {
      cache.updateQuery(
        { query: GET_TOPICS, variables: { mainId: mains.main?.id } },
        ({ getTopics }) => {
          return {
            getTopics: getTopics.filter(
              (topic: Topic) => response.data?.deleteTopic !== topic.id
            ),
          };
        }
      );
  ...
  const deleteTopicfunc = async (aiId: string, topicId: string) => {
    ...
    try {
      ...
      const updatedTopicList = topicList.filter(
        (arrayTopic: Topic) => arrayTopic.id !== topicId
      );

      if (mains.topic?.id === topicId) {
        setMains({
          ...mains,
          topic: { id: updatedTopicList[0].id, aiId: updatedTopicList[0].aiId },
        });
      }
    } catch (error) {
